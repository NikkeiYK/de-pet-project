...
# Справочники
PRODUCTS = ['Polypropylene homopolymer',
            'Polypropylene copolymer', 'Butadiene', 'Methanol']
REACTOR_TYPES = ['Polymerization', 'Extrusion',
                 'Distillation', 'Hydrogenation']
QUALITY_PARAMS = ['Melt Flow Index', 'Density',
                  'Ash Content', 'Volatile Matter', 'Catalyst Residue']
DOWNTIME_REASONS = ['Coke formation', 'Blade wear',
                    'Catalyst depletion', 'Scheduled maintenance', 'Sensor failure']
...
def generate_batches(n, reactor_ids, product_list):
    ...
    for i in range(1, n + 1):
        batch_start = fake.date_time_between(
        ...
        data.append({
            ...
            'status': random.choices(['completed', 'rejected', 'reworked'], weights=[0.92, 0.05, 0.03])[0]
        })
    return pd.DataFrame(data)
    ...
    for start in range(0, n, chunk_size):
        ...
        for _ in range(end - start):
            ...
            chunk.append({
                ...
                'reactor_id': random.choice(reactor_ids),
                # 10% - холостой ход
                'batch_id': random.choice(batch_ids) if random.random() > 0.1 else None,
                'sensor_type': random.choice(['temperature', 'pressure', 'flow_rate', 'vibration']),
...
def generate_downtime_events(n, reactor_ids):
    ...
    for i in range(1, n + 1):
        ...
        duration_hours = random.choice(
            [0.5, 1, 2, 4, 8, 24, 72, 168])  # от 30 мин до недели
        ...
        data.append({
            ...
            'lost_production_tons': round(duration_hours * random.uniform(2, 10), 1),
            'estimated_loss_rub': round(duration_hours * random.uniform(50000, 500000), 2)
        })
    return pd.DataFrame(data)
